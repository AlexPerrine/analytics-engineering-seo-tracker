...
from scripts.ingest_ga4_engagement import ingest_ga4_engagement_data
from scripts.ga4_checks import check_ga4_engagement_data, check_ga4_engagement_schema
...
@dag(
    dag_id="Ingest_GA4_engagement_views_daily",
    description="A DAG that ingests Google Analytics user engagement into Iceberg",
...
def ingest_ga4_engagement_dag():
    ...
    check_data_task = PythonOperator(
        task_id="check_ga4_engagement_data",
        python_callable=check_ga4_engagement_data,
        provide_context=True,
    )

    check_user_schema_task = PythonOperator(
        task_id="check_ga4_engagement_schema",
        python_callable=check_ga4_engagement_schema,
        provide_context=True,
    )
