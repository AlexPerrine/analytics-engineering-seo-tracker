...
def ingest_g4a_user_data():
    ...
    schema=Schema(
        ...
        NestedField(field_id=4, name="deviceCategory", field_type=StringType(), required=True),
        NestedField(field_id=5, name="operatingSystem", field_type=StringType(), required=True),
        NestedField(field_id=6, name="browser", field_type=StringType(), required=True),
        NestedField(field_id=7, name="firstSessionDate", field_type=DateType(), required=True),
        NestedField(field_id=8, name="searchTerm", field_type=StringType(), required=True),
        NestedField(field_id=9, name="platform", field_type=StringType(), required=True),
        NestedField(field_id=10, name="totalUsers", field_type=IntegerType(), required=True),
        NestedField(field_id=11, name="activeUsers", field_type=IntegerType(), required=True),
        NestedField(field_id=12, name="engagementRate", field_type=FloatType(), required=True),
        NestedField(field_id=13, name="sessionsPerUser", field_type=FloatType(), required=True),
    ...
    iceberg_table=catalog.create_table_if_not_exists(
        identifier="alexperrine.raw_ga_users",
        schema=schema,
        partition_spec=spec
    )
    ...
    print(f"Ingested {len(rows)} rows into raw_ga_users")

    table=catalog.load_table("alexperrine.raw_ga_users")
